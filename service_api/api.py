...
def setup_state(
    ...
    global _job_id_counter, _input_queue, _result_map
...
async def embed(request: Request) -> JSONResponse:
    ...
    # We could add logic to parse the request and determine priority here, but
    # for simplicity we'll just prioritize everything the same.
    priority = lodis.small_priority_queue.LEAST_URGENT_PRIORITY

    # Queue up the texts for embedding.
    logger.info(
    ...
    # Wait for results from the embedding loop.
    results = []
